(ns disco-legal.main
  "Kotodama WASM entrypoint for the Disco_legal clean-room actor (L4) — Clojure port.
  ...
  facts (`disco_legal.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "disco_legal")
(def tier "L4")
...
(def entity-specs
  [{:entity "Matter"        :plural "matters"        :id-prefix "discoleg_mat"
    :fields [:clientId :name :status :openedAt]           :required [:name :status]
    :coerce {}                       :refs {}}
   {:entity "Document"      :plural "documents"      :id-prefix "discoleg_doc"
...
(defn emit-facts
  "EAVT facts for one record: {\"disco_legal.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "disco_legal-compat" :tier tier :entities entities} 200])
...
(defn start! [] :disco_legal-compat/ready)
